...
SliceTiming::SliceTiming(const char* code, const char* name) : RSTask(code, name)
{
}
...
char* SliceTiming::getCmd() {
    ...
    for ( vector<rsArgument*>::size_type j = 0; j != arguments.size(); j++ ) {
        ...
        if ( ! strcmp(arg->key, "in") ) {
            ...
            newCmd = rsString("");
            mean = arg->value;
        } else if ( !strcmp(arg->key, "repeat") ) {
            newCmd = rsStringConcat(" --repeat=", arg->value, NULL);
        ...
        } else if ( ! strcmp(arg->key, "sliceIndexing") ) {
            if ( !strcmp(arg->value, "bottom-up") ) {
                newCmd = rsString("");
            } else {
                newCmd = rsString(" --down");
            }
        } else if ( ! strcmp(arg->key, "odd") ) {
            newCmd = rsString(" --odd");
        } else {
